...
def visualize_cp_result2(confidence_intervals, prediction_result, selected_steps, ax, bg_img=None, extent=None):
    """
    Draw confidence interval circles and optionally "un-mist" the background inside each circle.
    
    Parameters:
    - confidence_intervals: array-like of radii per timestep.
    - prediction_result: dict mapping obj_id to array of shape (T,2).
    - selected_steps: list of timesteps to visualize.
    - ax: matplotlib Axes.
    - bg_img: (H,W,3) array for the background image (no alpha).
    - extent: [xmin, xmax, ymin, ymax] used when displaying the background image.
    """
    ...
    for obj_id, traj in prediction_result.items():
        for idx, step in enumerate(selected_steps):
            ...
            # redraw background inside the patch to "erase" mist
