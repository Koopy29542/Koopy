...
class ConformalController:
    ...
    def __call__(self, pos_x, pos_y, orientation_z, linear_x, angular_z, boxes, predictions, goal):
        paths, vels = self.generate_paths(pos_x, pos_y, orientation_z, n_skip=5) # 경로 생성
        # paths, vels = self.generate_paths_wheel_vel(pos_x, pos_y, orientation_z, linear_x, angular_z)
        safe_paths, vels = self.filter_unsafe_paths(paths, vels, boxes) # 충돌 경로 제외
        if safe_paths is None:
            # print('MPC infeasible')
            return None, {'feasible': False}
        else:
            path, vel = self.score_paths(safe_paths, vels, predictions, goal) # 경로마다 score 계산 + 최종 경로 선정 
            info = {
                'feasible': True,
