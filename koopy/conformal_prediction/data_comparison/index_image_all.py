    ...
    for file in os.listdir(test_dirpath):
        if file.endswith('.txt'):
            ...
            for t, y_model in enumerate(test_set_y_model[t_begin:120+t_begin]):
                ...
                bbox = ax.get_window_extent()  # BBox(x0, y0, x1, y1) in display pixels
                x0, y0, x1, y1 = map(int, (bbox.x0, bbox.y0, bbox.x1, bbox.y1))
                h = image.shape[0]
                crop = image[h - y1 : h - y0, x0 : x1, :]   
                image = cv2.cvtColor(crop, cv2.COLOR_RGBA2BGR)
                output_size=(800, 800) 
                resized = cv2.resize(image, output_size, interpolation=cv2.INTER_LINEAR)
                cv2.imwrite(os.path.join(video_dir,str(t)+".jpg"), resized)
...
if __name__ == '__main__':
    ...
    # Zara
    [15.13244069, 12.3864436, -0.02104651, -0.23865981999999877],
    [15.13244069, 12.3864436, -0.02104651, -0.23865981999999877],
]"""
    map_sizes = [
    # Eth
